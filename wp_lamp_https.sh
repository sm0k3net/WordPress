...
#installing ssl
echo -e "${YELLOW}Starting SSL installation & configuration...${NC}"

read -r -p "Do you want to configure HTTPS automatically? [y/N] " response
case $response in
    [yY][eE][sS]|[yY]) 
  ...
  echo -e "${RED}WARNING! SSL was not configured and your website is not secure!.${NC}"
...
RewriteCond %{HTTPS_HOST} ^www.$domain_name$
RewriteRule (.*)$ https://$domain_name/$1 [R=301,L]
